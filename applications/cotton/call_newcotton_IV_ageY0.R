...
###*##——————————————————————————————————————————————————————##
##2. to estimate
startpar <- function(yy, xx1, xx2, xx3, xx4, rr, inip = rep(1, 4)) {
  ...
  glom.coef <- coef(glom)
  olp <- glom.coef[1] + glom.coef[2] * xx1 + glom.coef[3] * xx2 +
    glom.coef[4] * xx3 + glom.coef[5] * xx4
  ...
  ell <- function(par) {
    ...
    # remove xx1 from the propensity score model
    tt <- par[1] + par[2] * xx2 + par[3] * xx3 + log(qx + exp(par[4]) * px)
  ...
  Grad <- function(par) {
    ...
    tem1 <- sum(tem0)
    tem2 <- colSums((matrix(tem0) %*% matrix(1, 1, 2)) * cbind(xx2, xx3))
...
estpar <- function(yy, xx1, xx2, xx3, xx4, rr, inip = rep(1, 4)) {
  ...
  ell <- function(par) {
    ...
    ss <- par[5] + par[6] * xx1 + par[7] * xx2 + par[8] * xx3 + par[9] * xx4
    ppy <- 1 / (1 + exp(-ss))
    
    tt <- alp + beta[1] * xx2 + beta[2] * xx3 + log(1 + (exp(gam) - 1) * ppy)
  ...
  Grad <- function(par) {
    ...
    tt <- alp + beta[1] * xx2 + beta[2] * xx3 + log(qx + exp(gam) * px)
    
    tem0 <- 1 - rr - 1 / (1 + exp(-tt))
    tem1 <- sum(tem0)
    tem2 <- colSums((matrix(tem0) %*% matrix(1, 1, 2)) * cbind(xx2, xx3))
    tem2 <- as.numeric(tem2)
    tem3 <- sum(tem0 * px * exp(gam) / (1 + (exp(gam) - 1) * px))
  ...
  ess <- epar[5] + epar[6] * xx1 + epar[7] * xx2 + epar[8] * xx3 + epar[9] * xx4
  ppy <- 1 / (1 + exp(-ess))
  
  nlp <- epar[1] + epar[2] * xx2 + epar[3] * xx3 + log(1 - ppy + exp(epar[4]) * ppy)
  pix <- 1 / (1 + exp(nlp))
  ...
  return(list(
    hatxi = as.numeric(epar[5:9]),
    hattheta = as.numeric(epar[1:4]),
    tstat = tstat,
...
boot <- function(data, nboot = 100, inip = rep(1, 4)) {
  yy  <- data[, 1]
  xx1 <- data[, 2]
  xx2 <- data[, 3]
  xx3 <- data[, 4]
  ...
  for (i in 1:nboot) {
    ...
    bnlp <- hattheta[1] + hattheta[2] * bxx2 + hattheta[3] * bxx3 +
      log(1 + (exp(hattheta[4]) - 1) * bhatppy)
    bp <- 1 / (1 + exp(bnlp))
...
### directly estimate asymptotic variance
asyVar_fun <- function(data, inip = rep(1, 4)) {
  ## data = cotton
  yy  <- data[, 1]
  xx1 <- data[, 2]
  xx2 <- data[, 3]
  ...
  ## propensity score model excludes xx1
  tt <- hattheta[1] + hattheta[2] * xx2 + hattheta[3] * xx3 +
    log(1 + (exp(hattheta[4]) - 1) * hatppy)
  pix <- 1 / (1 + exp(tt))
  
  ## theta part only includes xx2 and xx3
  rx <- cbind(xx2, xx3)
  dim_rx <- ncol(rx)
  ...
  ## theta has 4 dims, xi has 5 dims
  dim_theta <- 4 + 5
  ...
  ##
  ## CI for theta only: theta has dim_rx + 2 = 4 components
  ...
  rownames(ci_theta) <- c("alp", paste0("beta", 1:dim_rx), "gam")
...
asyres=asyVar_fun(cotton,inipmat[minidx_asym,1:4])
tab_ci_theta = asyres$ci_theta



#### save resulta 
DGP_type_model = "IV_agey0/results_pvalue"
###
...
minidx_boot     0.952 0.4916525     616.0993     616.0993
minidx_asym     0.952 0.4916525     616.0993     616.0993
> tab_ci_theta
           est        SE     Wald Z    p-value        low         upp
alp   -1.72897540 0.2052570 -8.4234652 0.00000000 -2.1312792 -1.32667163
beta1 -0.43933050 0.2046251 -2.1470024 0.03179309 -0.8403956 -0.03826538
beta2  0.08864527 0.1924775  0.4605487 0.64512246 -0.2886107  0.46590124
gam    0.72259705 0.9630107  0.7503520 0.45304271 -1.1649039  2.61009803
...
# for (i in 1:lenini) {
#   asyres=asyVar_fun(cotton,inipmat[i,c(1:4)])
#   asyout=asyres$out
#   pval_asy = 1 - pnorm(asyout[1],mean = 0, sd =sqrt(asyout[2]))
...
# 
# asyoptim =asyVar_fun(cotton,inipmat[39,1:4])
